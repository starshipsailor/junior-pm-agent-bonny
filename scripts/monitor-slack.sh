#!/bin/bash
# =============================================================================
# Bonny Slack Monitor — Thin runner that invokes the Slack monitor skill
# =============================================================================
# Runs every 30 minutes via launchd. Reads watermark, checks budget, invokes
...
LOCKFILE="$STATE_DIR/slack-monitor.lock"
WATERMARK_FILE="$STATE_DIR/slack-monitor-last-run.json"
...
if [ -f "$LOCKFILE" ]; then
  ...
  if kill -0 "$PID" 2>/dev/null; then
    echo "$(date '+%Y-%m-%d %H:%M:%S') - Slack monitor already running (PID $PID). Skipping."
    exit 0
  fi
  rm -f "$LOCKFILE"
fi
...
# --- Build prompt ---
PROMPT="You are the Bonny Slack Monitor agent.

Read and execute the skill file at .claude/commands/skills/slack-monitor-skill.md

Time window: FROM_TIMESTAMP=$FROM_TIMESTAMP to NOW=$NOW
If FROM_TIMESTAMP is null, scan the last 30 minutes.

Current time: $(date '+%Y-%m-%d %H:%M')

Execute all steps in the skill file. Write actionable items to Mission_Control/inbox-slack.md."

# --- Run Claude Code ---
LOG_FILE="$LOG_DIR/${MONITOR_NAME}-$(date '+%Y-%m-%d_%H%M%S').log"

echo "$(date '+%Y-%m-%d %H:%M:%S') - Starting Slack monitor..."
...
echo "$(date '+%Y-%m-%d %H:%M:%S') - Slack monitor completed (exit code: $EXIT_CODE)."
